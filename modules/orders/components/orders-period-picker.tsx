...
export function OrdersPeriodPicker({
  ...
  return (
    <form action="/dashboard/orders" className="flex flex-wrap items-center gap-2 lg:flex-nowrap">
      <input type="hidden" name="period" value={period} />
      <div className="inline-flex min-h-12 items-center gap-1.5 rounded-full border border-red-950/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(255,248,248,0.92))] py-1.5 pl-2 pr-3 text-zinc-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.85),0_10px_24px_rgba(127,29,29,0.06)]">
        {showDay ? (
          <PickerButton id="day" label="День заказов" value={day} options={dayOptions} isOpen={openPicker === "day"} onOpen={() => togglePicker("day")} onChange={(value) => { setDay(value); setOpenPicker(null); }} />
        ) : null}
        {showMonth ? (
          <PickerButton id="month" label="Месяц заказов" value={month} options={dateParts.months} isOpen={openPicker === "month"} onOpen={() => togglePicker("month")} onChange={(value) => { setMonth(value); setOpenPicker(null); }} wide />
        ) : null}
        <PickerButton id="year" label="Год заказов" value={year} options={yearOptions} isOpen={openPicker === "year"} onOpen={() => togglePicker("year")} onChange={(value) => { setYear(value); setOpenPicker(null); }} />
