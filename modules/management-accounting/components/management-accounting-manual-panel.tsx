...
function ManualEntryList({
  ...
  entries: ManagementAccountingManualEntry[];
  canEdit: boolean;
}) {
  return (
    ...
    <GlassPanel className="p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <p className="rounded-full border border-red-100 bg-white/80 px-3 py-1 text-xs font-semibold text-red-800">
          {canEdit ? `${accounting.manualEntries.length} статей` : "Редактирование закрыто"}
        </p>
      </div>
      {!canEdit ? (
      ...
      <ManualEntryList entries={accounting.manualEntries} canEdit={canEdit} />
    </GlassPanel>
  );
}
